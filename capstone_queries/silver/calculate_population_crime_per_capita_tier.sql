CREATE TABLE academy.tayloro.colorado_crime_per_capita_rank AS
WITH crime_per_capita_per_year AS (
    -- Step 1: Compute Crime Per Capita for Each Year (1997-2000)
    SELECT
        LOWER(TRIM(c.county_name)) AS county, -- Standardize county name
    ...
    JOIN academy.tayloro.colorado_total_population_per_county_1997_2000 p
        ON LOWER(TRIM(c.county_name)) = LOWER(TRIM(p.county)) -- Match counties with case-insensitivity and trimming
        AND EXTRACT(YEAR FROM c.incident_date) = p.year
    WHERE p.total_population > 0 -- Avoid division by zero
    GROUP BY LOWER(TRIM(c.county_name)), EXTRACT(YEAR FROM c.incident_date), p.total_population
),
...
crime_tiers AS (
    -- Step 4: Assign Crime Per Capita Tiers
    SELECT
        county,
        avg_crime_per_1000,
        crime_percentile,
